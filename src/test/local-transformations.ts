...
function convertInputDataRowToData(inputData: {
    [index: string]: any
}): Data {
    ...
    return covariates
        .map((covariate) => {
            if(covariate.derivedField) {
                ...
                const inputDataWithLeadNodeColumns = testingDataTable
                    .map((testingDataRow) => {
                        return Object.keys(testingDataRow)
                            .reduce((inputDataForCovariate, testingDataRowColumn) => {
                                if(leafNodesForDerivedField
                                    .indexOf(testingDataRowColumn) > -1) {
                                    return Object.assign({}, inputDataForCovariate, {
                                        [testingDataRowColumn]: testingDataRow[testingDataRowColumn]
                                    })
                                } else {
                ...
                const inputData = inputDataWithLeadNodeColumns
                    .map(inputData => convertInputDataRowToData(inputData));

                const outputData = testingDataTable
                    .map((testingDataTableRow) => {
...
function assertionsForTestingObjects(
    ...
    testingObjects
        .forEach(({derivedField, inputData, outputData}) => {
            inputData.forEach((data, index) => {
                const actualOutput = calculateCoefficent(
                    derivedField, data, userFunctions
                );
...
describe.only(`Testing local transformations`, async function() {
    fs.readdirSync(`${AssetsDir}/algorithms`)
        .forEach((algorithmName) => {
            ...
            it(`Testing transformations for algorithm ${algorithmName}`, async function() {
                ...
                .buildFromAssetsFolder(algorithmAssetsDir))
                .toJson();
                ...
                if(model.modelType === ModelType.SingleAlgorithm) {
                    ...
                    const testingObjects = getTestObjectForSingleAlgorithmModel(model, testingData);
                    ...
                    const testingObjectsForGenders = getTestObjectForMultipleAlgorithmModel(model, testingData);

                    genderData
                        .forEach((genderDatum, index) => {
                            assertionsForTestingObjects(
