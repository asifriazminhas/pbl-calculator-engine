...
describe(`Testing local transformations`, function() {
    ...
    function runLocalTransformationsTests() {
        const localTransformationTestingFiles = path
            .join(`${assetsDir}/local-transformations`)
        fs.readdirSync(localTransformationTestingFiles)
        .filter(fileName => fileName !== '.DS_Store')
        .forEach((csvTestingFile) => {
            before(async function() {
                ...
                const localTransformationsTestingData: Array<{
                    ...
                    fs.readFileSync(`${localTransformationTestingFiles}/${csvTestingFile}`),
                    {
                        columns: true
                    }
                );
            ...
            it(`should properly calculate the local transformations for file ${csvTestingFile}`, function() {
                const cox: Cox = this.cox;
