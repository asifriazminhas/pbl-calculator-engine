...
import {
    calculateDataToCalculateCoefficent as calculateDataToCalculateCoefficentForRcsCustomFunction,
    calculateCoefficent as calculateCoefficentForRcsCustomFunction,
} from './custom-functions/rcs-custom-function';
import {
    calculateDataToCalculateCoefficent as calculateDataToCalculateCoefficentForDerivedField,
    calculateCoefficent as calculateCoefficentForDerivedField,
} from '../derived-field';
...
function formatCoefficent(covariate: Covariate, coefficent: any): number {
    ...
    if (!datumFound) {
        ...
        if (covariate.customFunction) {
            ...
            try {
                ...
                if (shouldLogWarnings()) {
                    console.warn(
                        `Incomplete data to calculate coefficent for data field ${covariate.name}. Setting it to reference point`,
                    );
                }
