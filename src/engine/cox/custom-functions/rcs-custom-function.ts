...
export function calculateCoefficent(
    ...
    const datumValue = getDataToCalculateCoefficent(
        rcsCustomFunction,
        data
...
function getDataToCalculateCoefficent(
    ...
    if (datumFound) {
        ...
        throw new Error(
            `No Datum found for Predictor with name ${rcsCustomFunction.firstVariableCovariate.name}`
        );
    }
}
