...
function mergeArrays(getUniqueIdProperty, arrayOne, arrayTwo) {
    ...
    arrayTwo.forEach(arrayTwoItem => {
        const indexOfArrayOneItemWithSameUniqueId = arrayOne.findIndex(arrayOneItem => getUniqueIdProperty(arrayOneItem) ===
    ...
    arrayOne.forEach(arrayOneItem => {
        mergedArray.find(mergedArrayItem => getUniqueIdProperty(mergedArrayItem) ===
