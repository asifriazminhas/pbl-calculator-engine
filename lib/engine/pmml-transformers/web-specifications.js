...
function addGeneralRegressionModelNode(pmmlNode, webSpecificationsCsv, baselineHazard, outputName) {
...
function transformPhiatDictionaryToPmml(phiatCsvString, webSpecificationsCategories, gender, addMeans, addBetas, baselineHazard) {
    addBetas;
    //Parse the csv string into array of objects
    const phiatCsv = csvParse(phiatCsvString, {
        columns: true
    });
    ...
    if (addBetas) {
        addGeneralRegressionModelNode(pmmlXml, phiatCsvRowsWithoutReference, baselineHazard, 'unknown');
        pmmlXml
            .ele('CustomPMML')
