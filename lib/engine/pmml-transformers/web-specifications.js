...
function addGeneralRegressionModelNode(pmmlNode, webSpecificationsCsv, baseline, outputName) {
...
function transformPhiatDictionaryToPmml(algorithmName, phiatCsvString, webSpecificationsCategories, gender, addMeans, addBetas, baseline) {
    addBetas;
    //Parse the csv string into array of objects
    const originalPhiatCsv = csvParse(phiatCsvString, {
        columns: true,
    });
    const phiatCsv = originalPhiatCsv.filter(phiatCsv => phiatCsv.Tool === algorithmName);
    ...
    if (addBetas) {
        addGeneralRegressionModelNode(pmmlXml, phiatCsvRowsWithoutReference, baseline, 'unknown');
        pmmlXml.ele('CustomPMML').ele('RestrictedCubicSpline');
    }
    ...
    Object.keys(dataFields).forEach(dataField => {
        ...
        if (dataFields[dataField].variableType === 'continuous') {
            ...
            if (relevantWebSpecificationCategory) {
                ...
                throw new Error(`No categories found for DataField ${dataField}`);
            }
        }
    });
...
//# sourceMappingURL=web-specifications.js.map