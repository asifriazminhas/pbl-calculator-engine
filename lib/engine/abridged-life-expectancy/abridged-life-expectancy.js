...
var AbridgedLifeExpectancy = /*#__PURE__*/function (_LifeExpectancy) {
  ...
  _createClass(AbridgedLifeExpectancy, [{
    ...
    value: function calculateForIndividual(individual) {
      ...
      var ageMaxAllowableValue = this.getMaxAge(ageContField); // Get the index of the life table row after which we need to
      // stop calculating values

      var lastValidLifeTableRowIndex = this.getLastValidLifeTableRowIndex(lifeTableWithQx, ageMaxAllowableValue);
      var completeLifeTable = this.getCompleteLifeTable(lifeTableWithQx, ageMaxAllowableValue, [lifeTableForIndividual[lastValidLifeTableRowIndex].age_start, lifeTableForIndividual[lastValidLifeTableRowIndex - 1].age_start]);
