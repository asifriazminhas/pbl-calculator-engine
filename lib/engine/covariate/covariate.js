...
function getComponent(covariate, data, userFunctions, tables) {
    if (env_1.shouldLogWarnings()) {
        console.groupCollapsed(`${covariate.name}`);
    }
    const component = calculateComponent(covariate, calculateCoefficent(covariate, data, userFunctions, tables));
...
function calculateCoefficent(covariate, data, userDefinedFunctions, tables) {
    const coefficentData = covariate.fieldType === field_1.FieldType.InteractionCovariate
        ? interaction_covariate_1.calculateDataToCalculateCoefficent(covariate, data, userDefinedFunctions, tables)
        : base_covariate_1.calculateDataToCalculateCoefficent(covariate, data, userDefinedFunctions, tables);
    let coefficent = 0;
    if (coefficentData.length === 1 &&
        ...
        coefficent = derived_field_1.calculateCoefficent(covariate.derivedField, coefficentData, userDefinedFunctions, tables);
    }
    const formattedCoefficent = data_1.formatCoefficentForComponent(coefficent, covariate);
