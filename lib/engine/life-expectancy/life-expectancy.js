  ...
  _createClass(LifeExpectancy, [{
    ...
    value: function getCompleteLifeTable(refLifeTableWithQxAndNx, maxAge) {
      ...
      completeLifeTable.reverse().forEach(function (lifeTableRow, index) {
        ...
        lifeTableRow.Tx = index === 0 ? _this.getFirstTxValue(completeLifeTable, maxAge) : completeLifeTable[index - 1].Tx + lifeTableRow.Lx; // ex = Tx/lx or use the ex value from the life table if we
        // cannot calculate it for this algorithm if this life table
        // row's age is greater than its maxAge

        lifeTableRow.ex = lifeTableRow.age > maxAge ? lifeTableRow.ex : lifeTableRow.Tx / lifeTableRow.lx;
      }); // Reverse it again
