...
var CoxSurvivalAlgorithm = /*#__PURE__*/function (_RegressionAlgorithm) {
  ...
  _createClass(CoxSurvivalAlgorithm, [{
    ...
    value: function getSurvivalToTimeWithBins(data, time) {
      ...
      _debugRisk.debugRisk.addEndDebugInfo(this.covariates, data, score, 1 - survival, NaN);
    ...
    value: function getRiskToTimeWithoutBins(data, time) {
      ...
      var predictionTimeInDays = formattedTime.diff((0, _moment.default)().startOf('day'), 'days');
      var baselineHazard = this.baseline.getBaselineHazard(predictionTimeInDays); // baseline*calibration*e^score

      var exponentiatedScoreTimesBaselineTimesCalibration = baselineHazard * this.calibration.getCalibrationFactorForData(data) * Math.pow(Math.E, score); // 1 - e^(-previousValue)

      var maximumTimeRiskProbability = 1 - Math.pow(Math.E, -exponentiatedScoreTimesBaselineTimesCalibration);

      _debugRisk.debugRisk.addEndDebugInfo(this.covariates, data, score, maximumTimeRiskProbability, baselineHazard);
