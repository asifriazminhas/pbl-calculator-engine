...
const pmml_functions_1 = require("../../../../engine/data-field/derived-field/pmml-functions");
...
const ApplyOperatorExceptions = {
    ...
    '-': function (apply, userDefinedFunctionNames, wrapFieldRefInMemberExpressionAst) {
        if (!apply.$$[1]) {
...
function getASTForFieldRef(fieldRef, wrapInMemberExpressionAst) {
    ...
    return wrapInMemberExpressionAst
        ? ast_1.getMemberExpressionAST(ast_1.getLiteralAST(fieldRef.$.field), 'obj')
        : ast_1.getIdentifierAST(fieldRef.$.field);
}
...
function getASTForApply(apply, userDefinedFunctionNames, wrapFieldRefInMemberExpressionAst) {
    ...
    else if (SpecialFunctions.find(specialFunction => specialFunction === apply.$.function)) {
...
const SpecialFunctions = Object.keys(pmml_functions_1.default);
/**
...
function getASTForUserDefinedFunctionApply(apply, userDefinedFunctionNames, wrapFieldRefInMemberExpressionAst) {
    ...
    //We make the function call look like func[apply.$.function] so that we can dynamically make the functions available at runtime
    return ast_1.getCallExpressionAST(ast_1.getMemberExpressionAST(ast_1.getLiteralAST(apply.$.function), 'userFunctions'), 
    //Go through all the function arguments
    (apply.$$
        ? apply.$$.map(apply => {
            switch (apply['#name']) {
        ...
        : []).concat(additionalFunctionPassedInArgs.map(functionArg => ast_1.getIdentifierAST(functionArg))));
}
...
function getPropertyAstFromFieldColumnPair(fieldColumnPair) {
    return ast_1.getPropertyAst(fieldColumnPair.$.column, fieldColumnPair.$.field
        ? ast_1.getMemberExpressionAST(ast_1.getLiteralAST(fieldColumnPair.$.field), 'obj')
        : ast_1.getLiteralAST(fieldColumnPair.$.constant));
}
function getAstForMapValues(mapValues) {
    const fieldColumnPairs = mapValues.FieldColumnPair instanceof Array
        ? mapValues.FieldColumnPair
        : [mapValues.FieldColumnPair];
    return ast_1.getCallExpressionAST(ast_1.getIdentifierAST('getValueFromTable'), [
        ast_1.getMemberExpressionAST(ast_1.getLiteralAST(mapValues.TableLocator.$.name), 'tables'),
        ast_1.getLiteralAST(mapValues.$.outputColumn),
        ast_1.getObjectExpressionAst(fieldColumnPairs.map(fieldColumnPair => {
