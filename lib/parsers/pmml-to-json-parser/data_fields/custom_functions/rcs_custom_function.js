...
function parseFirstVariableName(parameterLabel) {
  ...
  would return age */
  var variableNameWithoutModifiers = parameterLabel.split('_')[0];
  ...
  var modifiersWithoutRcsModifier = ['rcs1'].concat(parameterLabel // Split the string by _
  .split('_') // Remove the first entry since it has the variable name
  .slice(1) // Remove the rcs modifier
  .filter(function (modifierName) {
    return modifierName.indexOf('rcs') === -1;
  })) // Join the modifiers by _
  .join('_');
  return "".concat(variableNameWithoutModifiers, "_").concat(modifiersWithoutRcsModifier);
}
/**
