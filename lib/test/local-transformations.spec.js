...
function testCovariateTransformations(covariate, inputData, expectedOutputs, userFunctions) {
    ...
    inputData.forEach((currentInputData, index) => {
        ...
        chai_1.expect(diffError < 0.00001 || diffError === 0, `DerivedField ${derivedField.name}. Input Row: ${index +
            2}. Actual Output: ${actualOutput}. ExpectedOutput: ${expectedOutput}. DiffError: ${diffError}`).to.be.true;
    });
}
