...
function getTestingDataForCovariate(covariate, allData) {
  if (covariate.derivedField) {
    var leafFields = derived_field_1.getLeafFieldsForDerivedField(covariate.derivedField);
    ...
    var expectedOutput = formatTestingDataCsvColumn(data_1.findDatumWithName(covariate.name, allData).coefficent);
    return {
      inputData: inputData,
...
function testCovariateTransformations(covariate, inputData, expectedOutput, userFunctions, tables) {
  ...
  data_1.isEqual; // tslint:disable-next-line
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            test_utils_1.runIntegrationTest('local-transformations', 'local-transformations', 'Local Transformations', ['Sodium', 'SPoRT', 'RESPECT'], function (algorithm, data) {
